    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved list of orders",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = SupperOrderDTO.class))),
            @ApiResponse(responseCode = "404", description = "No orders found for the user",
                    content = @Content)
    })
    public ResponseEntity<List<SupperOrderDTO>> getOrders(
            @Parameter(hidden = true)
            @AuthenticationPrincipal UserDetails userDetails
            ){
        String username = userDetails.getUsername();

        List<SupperOrderDTO> ordersByUserEmail = supperOrderService.getOrdersByUserEmail(username);

        if(ordersByUserEmail.isEmpty()){
            return ResponseEntity.notFound().build();
        }

        return ResponseEntity.ok(ordersByUserEmail);
    }
}
